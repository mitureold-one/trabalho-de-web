...
export default function ColorController() {
  ...
  useEffect(() => {
    ...
    Object.entries(newColors).forEach(([key, value]) => {
      ...
      // Adicionamos a Quaternária aqui para ela também gerar o RGB
      if (key === "--cor-secundaria" || key === "--cor-terciaria" || key === "--cor-quaternaria") {
        const r = parseInt(value.slice(1, 3), 16);
        const g = parseInt(value.slice(3, 5), 16);
        const b = parseInt(value.slice(5, 7), 16);
        
        // Isso permite usar rgba(var(--cor-quaternaria-rgb), 0.8) no CSS
